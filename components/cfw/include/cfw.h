...
uint32_t CFW_nvSetCopsTimer(uint8_t nValue, CFW_SIM_ID nSimID);
uint32_t CFW_nvGetCopsTimer(CFW_SIM_ID nSimID);
...
typedef struct _CFW_NW_JAMMING_DETECT_GET_V2_IND
{
    uint8_t rat;     /*2--GSM;4--LTE*/
    uint8_t jamming; /*Jamming Status, 1 means Jamming detected*/
    uint8_t reserved[2];
} CFW_NW_JAMMING_DETECT_GET_V2_IND;
...
#define CFW_DISABLE_LTE_LIST_MAX_NUM 16
typedef struct
{
    uint8_t plmnNum;
    uint8_t regFailMaxNum;
    uint8_t supportInRoaming;
    uint8_t plmn[CFW_DISABLE_LTE_LIST_MAX_NUM * 6];
} CFW_DISABLE_LTE_PARA_T;

typedef struct
{
    uint8_t cause;
} CFW_REPORT_DISABLE_LTE_IND_T;

uint32_t CFW_NwSetDisableLtePara(CFW_DISABLE_LTE_PARA_T *pDisableLtePara, CFW_SIM_ID nSimID);
...
#define OPEN_CHANNEL_BEARER_GPRS 02
#define OPEN_CHANNEL_BEARER_DEFAULT 03
#define OPEN_CHANNEL_BEARER_E_UTRAN 0x0b //add by quectel, not useful currentlly, unisoc not finished!
...
uint32_t CFW_EmodSetReselectInTransfer(bool ReselectFlag);
...
uint32_t CFW_SetRFFreqPwrRange_V2(uint16_t mode, uint16_t band, int powerlow, int powerhigh);
uint32_t CFW_GetRFFreqPwrRange_V2(uint16_t mode, uint16_t band, int *powerlow, int *powerhigh);
...
/**************************************************************
** Description: This api is used to set/get band or r/hplmn priority. disable or
**    skip RPLMN search network.
** parameters:
** mode: 0 set/get band, 1 set/get r/hplmn
** enable: 0 disable r/hplmn, 1 enable r/hplmn. priority
** bandNum: bandNum
** bandList: bandList
** return 0 is success, others error
**************************************************************/
typedef struct
{
    uint8_t mode;
...
uint32_t CFW_nvSetNwSearchPrior(CFW_NwSearchPrior nwSearchPrior, CFW_SIM_ID nSimID);
uint32_t CFW_nvGetNwSearchPrior(CFW_NwSearchPrior *nwSearchPrior, CFW_SIM_ID nSimID);
...
uint32_t CFW_GetSyncPPM(uint8_t *status, int32_t *ppm);

bool CFW_DecodeCDMAImsi(uint8_t *pImsi, uint8_t *pCdma_imsi_m);
...
uint32_t CFW_SetDefaultPcoControl(uint8_t value, CFW_SIM_ID nSimID); //value 1:PDN req with Attach no set pco; 0: PDN req with Attach no change
