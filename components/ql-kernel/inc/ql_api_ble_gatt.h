...
typedef struct
{
    ...
    unsigned char name[QL_BLE_MAX_REMOTE_NAME_LEN];    // scan name data
    ...
    unsigned char raw_data[QL_BLE_ADV_SCAN_DATA_SIZE]; // scan raw data
...
typedef enum
{
    QL_RESERVED_SERVICE_DEL = 0, // 删除系统默认的GAP和GATT服务,建议不删除否则可能出现异常
