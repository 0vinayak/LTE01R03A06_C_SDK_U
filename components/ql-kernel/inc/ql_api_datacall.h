...
#define PROFILE_IDX_VOLTE_MAX           5//支持volte时候最大允许的数传pdp
#define PROFILE_IDX_MAX                 7//不支持volte时最大允许的数传pdp
...
typedef struct
{
	...
	struct v6_info v6;
    uint8_t auth_type;
    uint8_t apn_user[USERNAME_LEN_MAX+1];
    uint8_t apn_pwd[PASSWORD_LEN_MAX+1];
    uint8_t apn[APN_LEN_MAX+1];
}ql_data_call_info_s;
