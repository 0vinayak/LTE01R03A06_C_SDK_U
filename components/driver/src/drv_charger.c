...
#define _ENABLE_CHARGE_LOW_VOLT 0 //whether to trun off charging when battery voltage is very low
...
static uint32_t _drvChargerCheckVbatPresent(void)
{
    ...
    //modifed by ryan.yi 2022/9/5。新增可配置是否开启充电功能。
    //开启充电功能后，模块不接电池，充电的9秒内，VBUS为4.1V，不充电的1秒内，VBUS为4.3V，VBUS有电压波动
#ifdef CONFIG_QUEC_PROJECT_FEATURE
    if (true != ql_model_feature_ctx.charge_enable)
    {
        return 0;
    }
    else 
...
#else
            drvAdcGetChannelVolt(ADC_CHANNEL_PROG2ADC, ADC_SCALE_3V233);
#endif
    }
...
#if (_ENABLE_CHARGE_LOW_VOLT == 0)
        _drvChargerTurnOff();
#else
        if ((module_state.bat_statistic_vol > 3600) || (module_state.bat_cur_vol > 4000))
        {
            OSI_LOGI(0, "chg: off chg 1 sec");
            _drvChargerTurnOff();
        }
        else
        {
            OSI_LOGI(0, "chg: keep charging for low volt");
        }
#endif
    }
    else
...
static void _chgTaskEntry(void *param)
{
    ...
    if (module_state.chgmng_state == CHG_IDLE)
    {
        if (_drvChargerIsPresent()) //If we hadn't detected charger is pluged in in interrupts, this operator can ensure charging is ssarted.
        {
