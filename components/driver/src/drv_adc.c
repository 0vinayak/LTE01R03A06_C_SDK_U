...
static
#endif
void _adcBigScaleEffuseGet(void)
{
...
static
#endif
void _adcSmallScaleEffuseGet(void)
{
    uint32_t efuse_cal_data[2] = {0};
...
static
#endif
void _adcScale01EffuseGet(void)
{
    uint32_t efuse_cal_data[2] = {0};
...
static
#endif
void _adcScale02EffuseGet(void)
{
    uint32_t efuse_cal_data[2] = {0};
...
static
#endif
void _adcheadmicEffuseGet(void)
{
    uint32_t efuse_cal_data[2] = {0};
...
static
#endif
int32_t _adcChangAdcToVol(uint16_t channel, int32_t scale, int32_t con_mode, int32_t adc_value)
{
    uint32_t result;
    uint32_t vol = 0;
...
{
    int32_t value;
    REG_RDA2720M_GLOBAL_LDO_CH_CTRL_T ldo_ch_ctrl;
    if (channel == ADC_CHANNEL_LDO_CALOUT0)
        REG_ADI_CHANGE1(hwp_rda2720mGlobal->ldo_ch_ctrl, ldo_ch_ctrl, ldo_cal, reg);
    else if (channel == ADC_CHANNEL_LDO_CALOUT1)
        REG_ADI_CHANGE1(hwp_rda2720mGlobal->ldo_ch_ctrl, ldo_ch_ctrl, ldo_cal1, reg);
    else if (channel == ADC_CHANNEL_LDO_CALOUT2)
        REG_ADI_CHANGE1(hwp_rda2720mGlobal->ldo_ch_ctrl, ldo_ch_ctrl, ldo_cal2, reg);
    value = drvAdcGetChannelVolt(channel, scale);
    return value;
}
