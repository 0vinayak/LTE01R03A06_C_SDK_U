...
static bool prvFlashSkipEraseCheck(bootSpiFlash_t *d, uint32_t offset, size_t size)
{
    ...
    for (int i = 0; i <= size / sizeof(uint32_t); i++)
    {
        if (f_addr[i] != pattern)
        {
            OSI_LOGI(0, "Flash to erase 0x%x/0x%x", f_addr, size);//quectel simple log
            return false;
        }
    }

    OSI_LOGI(0, "Flash not erase 0x%x/0x%x", f_addr, size);//quectel simple log
    return true;
}
...
static int prvFlashStart(fdlContext_t *d)
{
    ...
    else
    {
        ...
        OSI_LOGD(0, "flash erase nowait 0x%x/0x%x/%d", dn->start_address, fend - faddress, esize);//quectel simple log
    }

    if (esize < 0)
    {
        dn->flash_stage = FLASH_STAGE_NONE;
        OSI_LOGE(0, "flash fail 0x%x/0x%x", dn->start_address, dn->total_size);//quectel simple log
...
static void prvFlashPolling(fdlContext_t *d, unsigned received)
{
    ...
    else if (dn->flash_stage == FLASH_STAGE_WAIT_DATA)
    {
        ...
        OSI_LOGV(0, "flash write nowait 0x%x/0x%x", waddress, wsize);//quectel simple log
    ...
    else if (dn->flash_stage == FLASH_STAGE_WRITE)
    {
        ...
        {
            esize = bootSpiFlashEraseNoWait(dn->flash, faddress, fend - faddress);
            OSI_LOGD(0, "flash erase nowait 0x%x/0x%x/%d", faddress, fend - faddress, esize);//quectel simple log
        }
        dn->erase_address += esize;
        dn->flash_stage = FLASH_STAGE_ERASE;
    }
...
static int prvFlashEnd(fdlContext_t *d)
{
    ...
    {
        OSI_LOGE(0, "flash crc err,0x%x/0x%x/0x%x/0x%x",//quectel simple log
...
#else
static void prvPackageFile(cpioFile_t *f, fdlEngine_t *fdl)
#endif
{
...
#ifndef CONFIG_QUEC_PROJECT_FEATURE
    OSI_LOGXD(OSI_LOGPAR_SII, 0, "pkg file %s:%d/%u", name, isdir, f->data_size);//quectel simple log
    if (isdir)
    {
        vfs_mkpath(name, 0);
    }
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE_BOOT_FAT_RW
	QUEC_BOOT_LOG("pkg file %s:%d/%u", name, isdir, f->data_size);
#endif
    ...
    else if(strncmp(name, CONFIG_FS_SPI_EXT_NAND_FAT_MOUNT_POINT, 7) == 0) //"/exnand"
	{
...
static void prvDataStart(fdlEngine_t *fdl, fdlPacket_t *pkt, fdlContext_t *d)
{
    ...
    OSI_LOGI(0, "data start0x%x size0x%x", start_addr, file_size);//quectel simple log
    ...
    if (start_addr == PHASECHECK_LOGIC_ADDRESS)
    {
        ...
        {
            OSI_LOGE(0, "bad phasecheck size %d", file_size);//quectel simple log
    ...
    else if (start_addr == FIXNV_LOGIC_ADDRESS)
    {
        ...
        {
            OSI_LOGE(0, "bad fixnv size %d", file_size);//quectel simple log
            ERR_REP_RETURN(fdl, BSL_REP_DOWN_SIZE_ERROR);
        }
    ...
    {
        OSI_LOGE(0, "bad start addr 0x%x", start_addr);//quectel simple log
...
static void prvDataMidst(fdlEngine_t *fdl, fdlPacket_t *pkt, fdlContext_t *d)
{
    ...
    OSI_LOGD(0, "FDL data midst 0x%x/0x%x/0x%x/0x%x/0x%x",//quectel simple log
...
static void prvDataEnd(fdlEngine_t *fdl, fdlContext_t *d)
{
    ...
    OSI_LOGI(0, "FDL data end,0x%x/0x%x/0x%x/0x%x",//quectel simple log
             d->dnld.start_address, d->dnld.total_size, d->dnld.received_size,
             d->dnld.stage);
...
{
    OSI_LOGI(0, "FDL reset normal");//quectel simple log
...
static void prvPowerOffPolling(fdlContext_t *d)
{
    ...
    if (BOOT_DNLD_FROM_UART(d->device_type))
    {
        ...
        {
            OSI_LOGI(0, "FDL pwroff timeout", ms);//quectel simple log
            bootPowerOff();

            // ideally, we shouldn't come here
            OSI_LOGE(0, "FDL pwroff fail");//quectel simple log
    ...
    if (BOOT_DNLD_FROM_USERIAL(d->device_type))
    {
        ...
        {
            OSI_LOGI(0, "FDL dev not connect,pwroff");//quectel simple log
            bootPowerOff();
...
{
    OSI_LOGI(0, "FDL pwroff");//quectel simple log
...
static void prvSetBaud(fdlEngine_t *fdl, fdlPacket_t *pkt, fdlContext_t *d)
{
    ...
    OSI_LOGD(0, "set baud %d", baud); //quectel simple log
...
/* 
    ...
    Be careful,do not open this part of the code,if you have already opened CONFIG_QUEC_PROJECT_FEATURE_PROG_FW_FMTSYS in your target.config.
*/
