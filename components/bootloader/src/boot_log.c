...
#define QUEC_ONLY_ENABLE_LOGI    1
#else
#define QUEC_ONLY_ENABLE_LOGI    0
#endif
 ...
 */
//LOG_RAMCODE static 
void prvTraceIdBasic(unsigned tag, unsigned nargs, unsigned fmtid, va_list ap)
{
 ...
 */
//static 
void prvTraceEx(unsigned tag, unsigned partype, const char *fmt, va_list ap)
{
    osiTraceHeader_t header;
    unsigned fmt_len = strlen(fmt);
    unsigned fmt_len_aligned = OSI_ALIGN_UP(fmt_len + 1, 4);
 ...
 */
//static 
void prvTraceIdEx(unsigned tag, unsigned partype, unsigned fmtid, va_list ap)
{
    osiTraceHeader_t header;
    osiTraceParamInfo_t pari = prvParamCount(partype);
 ...
 * Application basic trace
 */
LOG_RAMCODE void boot_osiTraceBasic(unsigned tag, unsigned nargs, const char *fmt, ...)
{
    if (!gTraceEnabled)
        return;
 ...
 * Application basic trace, with ID
 */
LOG_RAMCODE void boot_osiTraceIdBasic(unsigned tag, unsigned nargs, unsigned fmtid, ...)
{
#if !QUEC_ONLY_ENABLE_LOGI
    if (!gTraceEnabled)
        return;
 ...
 * Application extended trace
 */
void boot_osiTraceEx(unsigned tag, unsigned partype, const char *fmt, ...)
{
#if !QUEC_ONLY_ENABLE_LOGI
    if (!gTraceEnabled)
        return;
 ...
 * Application extended trace, with ID
 */
void boot_osiTraceIdEx(unsigned tag, unsigned partype, unsigned fmtid, ...)
{
#if !QUEC_ONLY_ENABLE_LOGI
    if (!gTraceEnabled)
        return;
 ...
 */
void boot_osiTraceVprintf(unsigned tag, const char *fmt, va_list ap)
{
 ...
 * Application trace printf
 */
void boot_osiTracePrintf(unsigned tag, const char *fmt, ...)
{
    if (!gTraceEnabled)
        return;
...
//LOG_RAMCODE 
//static 
void prvTraIdBasic(unsigned nargs, unsigned fmtid, va_list ap)
{
    unsigned dlen = 4 + 4 * nargs;
 ...
 */
//static 
void prvTraEx(unsigned partype, const char *fmt, va_list ap)
{
    osiTraHeader_t header;
 ...
 */
//static 
void prvTraIdEx(unsigned partype, unsigned fmtid, va_list ap)
{
    osiTraHeader_t header;
    osiTraceParamInfo_t pari = prvParamCount(partype);
    osiBuffer_t *bufs = (osiBuffer_t *)alloca((pari.count + 2) * sizeof(osiBuffer_t));
 ...
 */
LOG_RAMCODE void boot_osiTraceTraBasic(unsigned nargs, const char *fmt, ...)
{
    if (!gTraceEnabled)
        return;
 ...
 * tra pub basic trace
 */
LOG_RAMCODE void boot_osiTracePubBasic(unsigned module, unsigned category, unsigned nargs, const char *fmt, ...)
{
    if (!gTraceEnabled ||
 ...
 * tra lte basic trace
 */
LOG_RAMCODE void boot_osiTraceLteBasic(unsigned module, unsigned category, unsigned nargs, const char *fmt, ...)
{
    if (!gTraceEnabled ||
 ...
 */
LOG_RAMCODE void boot_osiTraceTraIdBasic(unsigned nargs, unsigned fmtid, ...)
{
    if (!gTraceEnabled)
        return;
 ...
 */
LOG_RAMCODE void boot_osiTracePubIdBasic(unsigned module, unsigned category, unsigned nargs, unsigned fmtid, ...)
{
    if (!gTraceEnabled ||
        !(module & v_tra_pubModuleControl) ||
        !(category & v_tra_categoryControl))
 ...
 */
LOG_RAMCODE void boot_osiTraceLteIdBasic(unsigned module, unsigned category, unsigned nargs, unsigned fmtid, ...)
{
    if (!gTraceEnabled ||
        !(module & v_tra_lteModuleControl) ||
        !(category & v_tra_categoryControl))
 ...
 */
void boot_osiTraceTraEx(unsigned partype, const char *fmt, ...)
{
    if (!gTraceEnabled)
        return;
 ...
 */
void boot_osiTracePubEx(unsigned module, unsigned category, unsigned partype, const char *fmt, ...)
{
    if (!gTraceEnabled ||
        !(module & v_tra_pubModuleControl) ||
        !(category & v_tra_categoryControl))
 ...
 */
void boot_osiTraceLteEx(unsigned module, unsigned category, unsigned partype, const char *fmt, ...)
{
    if (!gTraceEnabled ||
        !(module & v_tra_lteModuleControl) ||
        !(category & v_tra_categoryControl))
 ...
 */
void boot_osiTraceTraIdEx(unsigned partype, unsigned fmtid, ...)
{
    if (!gTraceEnabled)
        return;
 ...
 */
void boot_osiTracePubIdEx(unsigned module, unsigned category, unsigned partype, unsigned fmtid, ...)
{
    if (!gTraceEnabled ||
        !(module & v_tra_pubModuleControl) ||
        !(category & v_tra_categoryControl))
 ...
 */
void boot_osiTraceLteIdEx(unsigned module, unsigned category, unsigned partype, unsigned fmtid, ...)
{
    if (!gTraceEnabled ||
        !(module & v_tra_lteModuleControl) ||
        !(category & v_tra_categoryControl))
 ...
 */
void boot_osiTraceSendTraData(uint8_t type, osiBuffer_t *bufs, unsigned count, unsigned dlen)
{
    if (!gTraceEnabled)
        return;
 ...
 */
void boot_sxs_SetFnStamp(uint32_t *fn_stamp)
{
 ...
 */
void boot_sxs_SetTraceLevel(uint8_t Id, uint16_t LevelBitMap)
{
 ...
 */
bool boot_osiTraceRawSend(uint8_t flowid, unsigned tag, const void *data, uint32_t len)
{
#ifdef CONFIG_KERNEL_HOST_TRACE
    osiTraceHeader_t header;
    osiBuffer_t bufs[2] = {
    ...
    bool ok = osiTraceBufPutMulti(bufs, 2, tlen);
    return ok;
#else
    return true;
#endif
 ...
 * It is called in BT log, and should be redesigned.
 */
void boot_SCI_TraceCapData(unsigned data_type, const void *src_ptr, uint32_t size)
{
    osiTraceRawSend(0x70, data_type, src_ptr, size);
}

OSI_DECL_STRONG_ALIAS(boot_osiTraceBasic, void osiTraceBasic(unsigned tag, unsigned nargs, const char *fmt, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceIdBasic,  void osiTraceIdBasic(unsigned tag, unsigned nargs, unsigned fmtid, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceEx, void osiTraceEx(unsigned tag, unsigned partype, const char *fmt, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceIdEx, void osiTraceIdEx(unsigned tag, unsigned partype, unsigned fmtid, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceVprintf,void osiTraceVprintf(unsigned tag, const char *fmt, va_list ap));
OSI_DECL_STRONG_ALIAS(boot_osiTracePrintf,void osiTracePrintf(unsigned tag, const char *fmt, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceTraBasic,  void osiTraceTraBasic(unsigned nargs, const char *fmt, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTracePubBasic,  void osiTracePubBasic(unsigned module, unsigned category, unsigned nargs, const char *fmt, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceLteBasic, void osiTraceLteBasic(unsigned module, unsigned category, unsigned nargs, const char *fmt, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceTraIdBasic,  void osiTraceTraIdBasic(unsigned nargs, unsigned fmtid, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTracePubIdBasic,  void osiTracePubIdBasic(unsigned module, unsigned category, unsigned nargs, unsigned fmtid, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceLteIdBasic, void osiTraceLteIdBasic(unsigned module, unsigned category, unsigned nargs, unsigned fmtid, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceTraIdEx, void osiTraceTraIdEx(unsigned partype, unsigned fmtid, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTracePubEx, void osiTracePubEx(unsigned module, unsigned category, unsigned partype, const char *fmt, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceLteIdEx,void osiTraceLteIdEx(unsigned module, unsigned category, unsigned partype, unsigned fmtid, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceSendTraData, void osiTraceSendTraData(uint8_t type, osiBuffer_t *bufs, unsigned count, unsigned dlen));
OSI_DECL_STRONG_ALIAS(boot_sxs_SetFnStamp, void sxs_SetFnStamp(uint32_t *fn_stamp));
OSI_DECL_STRONG_ALIAS(boot_sxs_SetTraceLevel, void sxs_SetTraceLevel(uint8_t Id, uint16_t LevelBitMap));
OSI_DECL_STRONG_ALIAS(boot_osiTraceRawSend,bool osiTraceRawSend(uint8_t flowid, unsigned tag, const void *data, uint32_t len));
OSI_DECL_STRONG_ALIAS(boot_SCI_TraceCapData,void SCI_TraceCapData(unsigned data_type, const void *src_ptr, uint32_t size));
