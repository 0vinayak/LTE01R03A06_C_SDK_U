...
typedef enum
{
    ...
    BT_STATE_ERR,           /**< BT is error(as close timeout/internal error), need reset by upper layer*/
} bt_state_t;
...
typedef struct
{
    UINT16 state; /**< state of scl connection*/
    UINT16 acl_handle; /**< handle of acl connection*/
    UINT16 sco_handle; /**< handle of SCO connectiion*/
    bdaddr_t addr;    /**< addr of peer device */
}bt_conn_t;
...
typedef struct
{
    ...
    void (*bond_key_changed_callback) (bt_bond_state_t state, bt_dm_device_t *device_info);    /**< Bond key changed callback function */
    ...
    void (*acl_disconnect_reason_callback) (UINT8 reason, bdaddr_t *addr);
} bt_callbacks_t;
...
typedef struct
{
    ...
    int (*set_product_role)(UINT8 is_phone);  // is_phone =1 means phone, is_phone=0 means headset
